...
| `--image-type` | No | `fedora-sway-atomic` | Image type to build (fedora-bootc, fedora-sway-atomic) |
...
| `--desktop-environment` | No | - | Deprecated - Sway only |
...
> **Note:** Only Sway-based variants are supported. Non-Sway desktop environments have been removed.
...
When requesting the `fedora-bootc` image type, the `client_payload` accepts
`window_manager` to regenerate the YAML with the requested desktop before validation.
Only Sway is supported as a window manager. The `desktop_environment` field is deprecated.
