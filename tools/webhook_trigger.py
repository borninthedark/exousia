...
def trigger_build(
    ...
    if yaml:
        ...
        if '\n' in yaml or yaml.strip().startswith(('name:', 'description:', 'modules:', 'base-image:')):
            ...
            allowed_roots = {Path("yaml-definitions"), Path("overlays")}
            if len(yaml_path.parts) > 1 and yaml_path.parts[0] not in {root.parts[0] for root in allowed_roots}:
                raise ValueError("YAML config path must stay within allowed directories (yaml-definitions/, overlays/)")
...
Security Notes:
  ...
  - Store tokens securely and never commit them to git
    ...
    parser.add_argument(
        ...
        help="Desktop environment (deprecated - Sway only)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
