...
- **OSV-Scanner** -- Dependency and source vulnerability scanning in CI
- **OpenSCAP** -- Image policy and compliance scanning in CI
- **Hadolint** -- Dockerfile/Containerfile best-practice linting
- **CodeQL** -- Advanced static analysis for Python and GitHub Actions
...
- AIDE baseline initialized during image assembly
...
- TDD with minimum 95% code coverage on Python tools (current: 96.03%)
...
Security workflows publish their evidence through GitHub-native artifacts and
summaries:

- `trivy-results.txt` is uploaded from Hiyori for image-scan review
- an SBOM is generated by Trivy and submitted to GitHub Dependency Graph on
  `main`
- OpenSCAP HTML/XML artifacts are uploaded from Hiyori for compliance review
- CodeQL alerts flow through GitHub code scanning
- STATUS snapshots are generated by Nemu for recent pipeline visibility
